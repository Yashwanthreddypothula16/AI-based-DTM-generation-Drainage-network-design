...
def main():
    ...
    sc1 = ax.scatter(
        gx, gy, gz * z_exag, color="#8B4513", s=0.5, alpha=0.4, depthshade=False
    )

    # Layer 2: Non-Ground Points
    ox, oy, oz = las.x[s_other], las.y[s_other], las.z[s_other]
    sc2 = ax.scatter(
        ox, oy, oz * z_exag, color="#32CD32", s=0.5, alpha=0.7, depthshade=False
    )

    ax.set_title(
        "3D ML Classification: Ground vs Structures",
        color="white",
        fontsize=16,
        fontweight="bold",
        pad=10,
