...
def main():
    ...
    ax.scatter(
        px, py, pz * z_exag, c=pz, s=1, cmap="inferno", alpha=0.3, depthshade=False
    )
    ...
    for geom in drainage.geometry:
        if geom.geom_type == "LineString":
            ...
            line_y = coords[:, 1]
            line_z = (
                np.array([get_z_height(x, y) for x, y in zip(line_x, line_y)]) * z_exag
            )
            ax.plot(
                line_x, line_y, line_z, color="cyan", linewidth=2, alpha=0.8, zorder=10
            )
    ...
    # Formatting
    ax.set_title(
        "3D Drainage Output Model: Integrated Point Cloud & Flow Strategy",
        color="white",
        fontsize=28,
        fontweight="bold",
        pad=40,
    ...
    brain_out = r"C:\Users\nanir\.gemini\antigravity\brain\e25be27b-f581-4821-a67d-2789942db877\MASTER_INTEGRATED_3D_DASHBOARD.png"
