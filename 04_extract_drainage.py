...
def main():
    ...
    dtm_filled = os.path.join(
        base_dir, r"Rajasthan_Point_Cloud\rajasthan_dtm_filled.tif"
    )
    ...
    streams_vector = os.path.join(
        base_dir, r"Rajasthan_Point_Cloud\rajasthan_drainage_network.shp"
    )
    ...
    # 4. Extract Streams
    threshold = (
        500  # Higher threshold because the village is bigger, we only want main drains
    )
    ...
    wbt.raster_streams_to_vector(
        streams=streams_raster, d8_pntr=flow_dir, output=streams_vector
    )
