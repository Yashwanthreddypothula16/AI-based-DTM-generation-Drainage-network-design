...
def main():
    ...
    plt.suptitle(
        "Rajasthan Village: AI-Driven Drainage & Flood Risk Analysis Dashboard",
        fontsize=32,
        fontweight="bold",
        y=0.95,
        color="#2c3e50",
    ...
    with rasterio.open(dtm_path) as src:
        ...
        ax_a.plot_surface(
            X, Y, np.flipud(dtm_data), facecolors=rgb, linewidth=0, antialiased=True
        )
        ax_a.view_init(elev=50, azim=45)
        ax_a.set_axis_off()
        ax_a.set_title(
            "A. Realistic 3D Terrain (AI Ground Classification)",
            fontsize=20,
            fontweight="bold",
            pad=20,
        )
    ...
    with rasterio.open(twi_path) as src:
        ...
        ax_b.set_title(
            "B. Classified Waterlogging & Flood Risk Zones",
            fontsize=20,
            fontweight="bold",
            pad=20,
        )
    ...
    with rasterio.open(flow_acc_path) as src:
        ...
        ax_c.set_title(
            "C. Drainage Flow Concentration (Logic Layer)",
            fontsize=20,
            fontweight="bold",
            pad=20,
        )
    ...
    with rasterio.open(dtm_path) as src:
        ...
        drainage.plot(
            ax=ax_d, color="blue", linewidth=1.5, label="Designed Drainage Lines"
        )
        ...
        with rasterio.open(flow_acc_path) as f_src:
            ...
            mask = np.ma.getmaskarray(f_data)
            fy, fx = np.where(~mask)
            for idx in f_flat_idx:
                ...
                ax_d.scatter(
                    ...
                    marker="*",
                    edgecolors="black",
                    label="Recommended Strategy Outlets"
                    if idx == f_flat_idx[-1]
                    else "",
                )
        ...
        with rasterio.open(flow_dir_path) as dir_src:
            ...
            step = 60
            r_idx, c_idx = np.mgrid[
                0 : d_data.shape[0] : step, 0 : d_data.shape[1] : step
            ]
            ...
            Up = np.array(
                [
                    [
                        dx_map.get(d_data[r, c], 0)
                        for c in range(0, d_data.shape[1], step)
                    ]
                    for r in range(0, d_data.shape[0], step)
                ]
            )
            Vp = np.array(
                [
                    [
                        dy_map.get(d_data[r, c], 0)
                        for c in range(0, d_data.shape[1], step)
                    ]
                    for r in range(0, d_data.shape[0], step)
                ]
        ...
        ax_d.set_title(
            "D. Final Strategy: Connected Network & Outlets",
            fontsize=20,
            fontweight="bold",
            pad=20,
        )
    ...
    plt.figtext(
        0.5,
        0.02,
        "This automated AI/ML workflow classifies point clouds, generates conditioned DTMs, and designs resilient drainage networks for densely inhabited village areas.",
        ha="center",
        fontsize=18,
        style="italic",
        color="#7f8c8d",
