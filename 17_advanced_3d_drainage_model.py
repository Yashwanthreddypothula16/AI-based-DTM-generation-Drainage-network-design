...
def main():
    ...
    with rasterio.open(acc_path) as a_src:
        ...
        for idx in flat_idx:
            ...
            if (
                0 <= d_row < dtm_data.shape[0] * 3
                and 0 <= d_col < dtm_data.shape[1] * 3
            ):
    ...
    for i in range(dtm_data.shape[1] - 1):
        v = [
            [
                (X[0, i], Y[0, i], Z[0, i] * z_exag),
                (X[0, i + 1], Y[0, i + 1], Z[0, i + 1] * z_exag),
                (X[0, i + 1], Y[0, i + 1], min_z * z_exag),
                (X[0, i], Y[0, i], min_z * z_exag),
            ]
        ]
    ...
    for i in range(dtm_data.shape[1] - 1):
        v = [
            [
                (X[-1, i], Y[-1, i], Z[-1, i] * z_exag),
                (X[-1, i + 1], Y[-1, i + 1], Z[-1, i + 1] * z_exag),
                (X[-1, i + 1], Y[-1, i + 1], min_z * z_exag),
                (X[-1, i], Y[-1, i], min_z * z_exag),
            ]
        ]
        ax.add_collection3d(Poly3DCollection(v, color="#333333", alpha=0.8))
    # West side
    for i in range(dtm_data.shape[0] - 1):
        v = [
            [
                (X[i, 0], Y[i, 0], Z[i, 0] * z_exag),
                (X[i + 1, 0], Y[i + 1, 0], Z[i + 1, 0] * z_exag),
                (X[i + 1, 0], Y[i + 1, 0], min_z * z_exag),
                (X[i, 0], Y[i, 0], min_z * z_exag),
            ]
        ]
    ...
    for i in range(dtm_data.shape[0] - 1):
        v = [
            [
                (X[i, -1], Y[i, -1], Z[i, -1] * z_exag),
                (X[i + 1, -1], Y[i + 1, -1], Z[i + 1, -1] * z_exag),
                (X[i + 1, -1], Y[i + 1, -1], min_z * z_exag),
                (X[i, -1], Y[i, -1], min_z * z_exag),
            ]
        ]
        ax.add_collection3d(Poly3DCollection(v, color="#222222", alpha=0.8))
    ...
    v_bot = [
        [
            (bounds.left, bounds.bottom, min_z * z_exag),
            (bounds.right, bounds.bottom, min_z * z_exag),
            (bounds.right, bounds.top, min_z * z_exag),
            (bounds.left, bounds.top, min_z * z_exag),
        ]
    ]
        ...
        if geom.geom_type == "LineString":
            ...
            for xi, yi in zip(lx, ly):
                ...
                if 0 <= sr < dtm_data.shape[0] and 0 <= sc < dtm_data.shape[1]:
                    lz.append(
                        dtm_data[sr, sc] * z_exag + 2
                    )  # Slightly elevated for visibility
                else:
                    lz.append(min_z * z_exag)
    ...
    ax.set_title(
        "3D Drainage Output Model: Physical Slab Edition",
        color="white",
        fontsize=28,
        fontweight="bold",
        pad=40,
    ...
    # Copy to artifacts
    brain_out = r"C:\Users\nanir\.gemini\antigravity\brain\e25be27b-f581-4821-a67d-2789942db877\ADVANCED_3D_DRAINAGE_SLAB_MODEL.png"
