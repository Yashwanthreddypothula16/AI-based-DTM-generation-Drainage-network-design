...
def get_dump_zones(flow_acc_path, n_points=5):
    ...
    y_idx, x_idx = np.where(~valid_data.mask)
...
def main():
    ...
    with rasterio.open(flow_acc_path) as src:
        ...
        show(
            ...
            title="Flow Accumulation (High Density Drainage Paths)",
        )
    plt.savefig(
        os.path.join(out_dir, "map1_flow_accumulation.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
    ...
    plt.savefig(
        os.path.join(out_dir, "map2_flow_direction.png"), dpi=300, bbox_inches="tight"
    )
    plt.close()
    ...
    with rasterio.open(twi_path) as src:
        ...
        show(
            ...
            ax=ax,
            cmap="YlGnBu",
            title="Waterlogging Flood Risk (Topographic Wetness Index)",
        )
        drainage_net.plot(ax=ax, color="red", alpha=0.6, linewidth=0.5)
    plt.savefig(
        os.path.join(out_dir, "map3_flood_risk_twi.png"), dpi=300, bbox_inches="tight"
    )
    plt.close()
    ...
    with rasterio.open(dtm_path) as dtm:
        show(
            ...
            ax=ax,
            cmap="terrain",
            title="Drainage Strategy with Targeted Outlet Dump Zones",
        )
        ...
        for px, py in dump_points:
            ax.scatter(
                ...
                marker="*",
                edgecolors="black",
                label="Recommended Dump Outlets",
            )
    ...
    plt.savefig(
        os.path.join(out_dir, "map4_drainage_dump_zones.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
    ...
    with rasterio.open(dtm_path) as src:
        ...
        rgb = ls.shade(
            np.flipud(dtm_data), cmap=plt.cm.terrain, vert_exag=2, blend_mode="overlay"
        )
    ...
    plt.savefig(
        os.path.join(out_dir, "map5_3D_realistic_hillshade.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
